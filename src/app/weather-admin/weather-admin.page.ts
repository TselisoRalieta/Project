...
@Component({
  ...
  templateUrl: './weather-admin.page.html',
  styleUrls: ['./weather-admin.page.scss'],
  standalone: false,
})
export class WeatherAdminPage  implements OnInit {
  today: Date = new Date();
  ...
  checkDangerousWeather(locationName: string, data: any) {
    ...
    if (temp <= this.coldTemperatureThreshold) {
      this.db.list('notifications').push({
        ...
        description: `Temperature is ${temp.toFixed(15)}°C. Dress warmly for your travel.`,
        timestamp
      });
    }
  }
